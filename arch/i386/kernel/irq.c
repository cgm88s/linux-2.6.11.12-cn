...
#ifdef CONFIG_4KSTACKS //当内核栈配置为4K时，内核使用3种不同的栈: 异常栈，硬中断栈，软中断栈
/*
...
static union irq_ctx *hardirq_ctx[NR_CPUS]; //硬中断请求栈
static union irq_ctx *softirq_ctx[NR_CPUS]; //软中断请求栈
#endif
 ...
 */
/** 
 * do_IRQ执行与一个中断相关的所有中断服务例程. 由common_interrupt调用，参数为SAVE_ALL压入栈中的值
 */
fastcall unsigned int do_IRQ(struct pt_regs *regs)
{	
	/* high bits used in ret_from_ code */
	int irq = regs->orig_eax & 0xff; //中断号
	...
	/**
	 * irq_enter增加中断嵌套计数，preempt_count
	 */
	irq_enter();
	 ...
	 * 如果中断栈使用不同的的栈,就需要切换栈.
	 */
	curctx = (union irq_ctx *) current_thread_info(); //获取与内核栈相连的thread_info描述符地址
