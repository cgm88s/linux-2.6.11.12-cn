 ...
 * we just go ahead and read them.
	 */
		...
		sec = CMOS_READ(RTC_SECONDS);	//0, 往 0x70端口写， 从0x71端口读
		min = CMOS_READ(RTC_MINUTES);	// 2
		hour = CMOS_READ(RTC_HOURS);	// 4
		day = CMOS_READ(RTC_DAY_OF_MONTH); // 7
		mon = CMOS_READ(RTC_MONTH);		// 8
		year = CMOS_READ(RTC_YEAR);		// 9
...
void __init time_init(void)
{
	...
	xtime.tv_sec = get_cmos_time();	//初始化系统时间
	xtime.tv_nsec = 0;
	...
	setup_irq(0, &irq0);	// 初始化0号中断，定时器中断，timer_interrupt
...
irqreturn_t hpet_rtc_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (AIE_on) {
		if ((curr_time.tm_sec == alarm_time.tm_sec) &&
			...
			call_rtc_interrupt = 1;
			rtc_int_flag |= RTC_AF;	//0x20
		}
	}
	if (call_rtc_interrupt) {
		rtc_int_flag |= (RTC_IRQF | (RTC_NUM_INTS << 8));  //0x80 |(1<<8)
