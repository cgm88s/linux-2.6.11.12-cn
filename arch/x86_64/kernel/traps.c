...
{
	int i;
	int in_kernel = (regs->cs & 3) == 0; //0级别为内核态
	unsigned long rsp;
	const int cpu = safe_smp_processor_id(); 
	struct task_struct *cur = cpu_pda[cpu].pcurrent;  //current

		rsp = regs->rsp;

	printk("CPU %d ", cpu);
	__show_regs(regs); //打印各寄存器的值
	...
	if (in_kernel) {
		...
		printk("Stack: ");
		show_stack(NULL, (unsigned long*)rsp); //打印调用栈
...
static DEFINE_SPINLOCK(die_lock);static arch_spinlock_t die_lock = __ARCH_SPIN_LOCK_UNLOCKED;
...
#endif
	printk("\n");
	notify_die(DIE_OOPS, (char *)str, regs, err, 255, SIGSEGV); //通die_chain链事情
	show_registers(regs);	//显示各寄存器的值，打印调用栈
...
{
	oops_begin();
	handle_BUG(regs); //打印bug所发生的位置  printk("----------- [cut here ] --------- [please bite here ] ---------\n");
											//printk(KERN_ALERT "Kernel BUG at %.50s:%d\n", f.filename, f.line);
	__die(str, regs, err); //打印详细bug原因，显示所有寄存器的值
	oops_end();    // panic("Oops");
	do_exit(SIGSEGV); //终止当前进程
}
			   ...
			   struct pt_regs * regs, long error_code, siginfo_t *info)
{
	...
	if ((regs->cs & 3)  != 0) { 
		...
		tsk->thread.error_code = error_code;	//保存数据码
		tsk->thread.trap_no = trapnr;			//保存中断向量号
		if (info)
			force_sig_info(signr, info, tsk);
		else
			force_sig(signr, tsk);		//向进程发送信号
		return;
	}
...
void __init trap_init(void)
{
	...
	set_intr_gate_ist(2,&nmi,NMI_STACK); //do_nmi
	...
	set_intr_gate_ist(18,&machine_check, MCE_STACK);  // MCE 硬件检测机制
#endif
