...
asmlinkage void do_page_fault(struct pt_regs *regs, unsigned long error_code)
{
	...
	/* get the address */
	__asm__("movq %%cr2,%0":"=r" (address)); //读取引起异常的线性地址。CPU控制单元把这个值存放在cr2控制寄存器中。
	if (notify_die(DIE_PAGE_FAULT, "page fault", regs, error_code, 14,
					SIGSEGV) == NOTIFY_STOP)
		return;

	if (likely(regs->eflags & X86_EFLAGS_IF))
		local_irq_enable();	// sti 开中断 IF=1
	 ...
	 */
	if (unlikely(address >= TASK_SIZE)) {   //内核态地址
		if (!(error_code & 5)) {		// & 0b 101	内核态访问了
 ...
 again:
	...
	*/
	...
	if (!vma)		// 如果vma为空，说明在出错地址后面没有线性区了，说明错误的地址肯定是无效的。
		goto bad_area;
	if (likely(vma->vm_start <= address)) 	// address 在 vma内
		goto good_area;
 ...
 * we can handle it..
 */
good_area:		//找到了一上正确的vma
	info.si_code = SEGV_ACCERR;
	write = 0;
	switch (error_code & 3) {
		default:	/* 3: write, present 写存在但无效的地址 */
			/* fall through */
		case 2:		/* write, not present 写不存在的地址*/
