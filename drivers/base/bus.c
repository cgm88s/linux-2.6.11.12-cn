...
{
	if (drv->bus->match && !drv->bus->match(dev, drv))  //scsi_bus_type->scsi_bus_match
		return -ENODEV;

	dev->driver = drv;	//sd_template.gendrv
	if (drv->probe) {
		int error = drv->probe(dev); sd_probe
		if (error) {
			dev->driver = NULL;
			return error;
		}
...
int device_attach(struct device * dev)
{
	...
	if (bus->match) {
		list_for_each(entry, &bus->drivers.list) {
			...
			error = driver_probe_device(drv, dev); //遍历总线上的驱动列表，关联此新加入的设备
			if (!error)
...
int bus_add_device(struct device * dev)
{
	...
	if (bus) {
		...
		list_add_tail(&dev->bus_list, &dev->bus->devices.list);  // 把设备添加到总线的设备列表
		device_attach(dev);		//把设备与驱动关联
