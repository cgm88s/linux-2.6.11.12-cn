...
int elv_merge(request_queue_t *q, struct request **req, struct bio *bio)
{
	...
	return ELEVATOR_NO_MERGE; // 0 表示不能合并，需要重新分配一个request, 挂到请求队列request_queue_t 
}
...
struct request *elv_next_request(request_queue_t *q)
{
	...
	while ((rq = __elv_next_request(q)) != NULL) {/* 从电梯队列中取出一个请求进行处理 */
		...
		ret = q->prep_rq_fn(q, rq);/* 需要进行一些SCSI命令准备工作 scsi_prep_fn*/
...
int elv_set_request(request_queue_t *q, struct request *rq, int gfp_mask)
{
	...
	if (e->ops->elevator_set_req_fn)	// 为请求分配调度算法相关的内存
		return e->ops->elevator_set_req_fn(q, rq, gfp_mask); // cfq_set_request
...
int elv_may_queue(request_queue_t *q, int rw)
{
	...
	if (e->ops->elevator_may_queue_fn)
		return e->ops->elevator_may_queue_fn(q, rw);	// cfq_may_queue
