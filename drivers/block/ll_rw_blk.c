...
{
	return q->nr_congestion_on;  //队列的阀值，超过就表示阻塞
}
...
void blk_plug_device(request_queue_t *q)
{
	...
	if (!test_and_set_bit(QUEUE_FLAG_PLUGGED, &q->queue_flags))
		mod_timer(&q->unplug_timer, jiffies + q->unplug_delay);  // 默认是 3ms, 超时函数 blk_unplug_timeout
}
...
static void blk_unplug_work(void *data)
{
	 ...
	 */
	q->unplug_fn(q);  generic_unplug_device
}
...
request_queue_t *blk_init_queue(request_fn_proc *rfn, spinlock_t *lock)
{
	...
	blk_queue_make_request(q, __make_request);  // q->make_request_fn = __make_request
						...
						int gfp_mask)
{
	...
	if (!elv_set_request(q, rq, gfp_mask)) //为请求分配调度算法相关的内存
		return rq;
...
/* 
 * Get a free request, queue_lock must not be held. 分配一个新的request挂入请求队列
 */
static struct request *get_request(request_queue_t *q, int rw, int gfp_mask)
{
	...
	if (unlikely(test_bit(QUEUE_FLAG_DRAIN, &q->queue_flags)))  //请法队列中标志 DRAIN
		goto out;
	...
	switch (elv_may_queue(q, rw)) { // 请求队列能否增回请求
		case ELV_MQUEUE_NO:	// 表式不能增加请求
			goto rq_starved;
		case ELV_MQUEUE_MAY:
			break;
	...
	if (blk_queue_full(q, rw) && !ioc_batching(q, ioc)) {	// 如果队列满，且IO没有优先级
		/*
...
get_rq:
	...
	rq = blk_alloc_request(q, rw, gfp_mask);  //分配一个request请求结构
	if (!rq) {
		/*
...
void drive_stat_acct(struct request *rq, int nr_sectors, int new_io)
{
	...
	if (rw == READ) {	// 读扇区计数增加
		__disk_stat_add(rq->rq_disk, read_sectors, nr_sectors); struct disk_stats
		if (!new_io)
			__disk_stat_inc(rq->rq_disk, read_merges);  //合并读IO计数，
	} else if (rw == WRITE) {	//写扇区计数增加
		__disk_stat_add(rq->rq_disk, write_sectors, nr_sectors);
		if (!new_io)
			__disk_stat_inc(rq->rq_disk, write_merges); //合并写IO计数
	}
	if (new_io) {	//新的请求
		disk_round_stats(rq->rq_disk);
		rq->rq_disk->in_flight++;	//请求个数+1
	}
}
...
{
	drive_stat_acct(req, req->nr_sectors, 1); //更新磁盘状态计数器
...
void disk_round_stats(struct gendisk *disk)
{
	...
	__disk_stat_add(disk, time_in_queue,
			disk->in_flight * (now - disk->stamp));  struct disk_stats
	disk->stamp = now;

	if (disk->in_flight)
		__disk_stat_add(disk, io_ticks, (now - disk->stamp_idle)); struct disk_stats
	disk->stamp_idle = now;
}
...
void __blk_put_request(request_queue_t *q, struct request *req)
{
	...
	if (rl) {
		...
		blk_free_request(q, req); //释放请求
		freed_request(q, rw);  //更新请求队列计数，唤醒等待下发request的进程
	}
}
...
get_rq:
	 ...
	 * 将bio插入请求链表。
	 */
	add_request(q, req);    //把新申请的request请求加入请求队列
out:
...
void submit_bio(int rw, struct bio *bio)
{
	...
	if (unlikely(block_dump)) {   // block_dump调试IO时打印信息
		char b[BDEVNAME_SIZE];
