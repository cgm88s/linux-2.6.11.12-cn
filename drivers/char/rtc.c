...
static DECLARE_WAIT_QUEUE_HEAD(rtc_wait); struct wait_queue *rtc_wait;

#ifdef RTC_IRQ
static struct timer_list rtc_irq_timer;	//RTC 定时器
#endif
...
static int rtc_do_ioctl(unsigned int cmd, unsigned long arg, int kernel)
{
	...
	{
		mask_rtc_irq_bit(RTC_PIE);	//禁止RTC中断
		if (rtc_status & RTC_TIMER_ON) {
			spin_lock_irq (&rtc_lock);
			rtc_status &= ~RTC_TIMER_ON;
			del_timer(&rtc_irq_timer);  // 删除RTC定时器
			spin_unlock_irq (&rtc_lock);
		}
		return 0;
	}
	case RTC_PIE_ON:	/* Allow periodic ints		使能RTC时间定时器*/
	{
		...
		if (!(rtc_status & RTC_TIMER_ON)) {
			...
			add_timer(&rtc_irq_timer);	//添加RTC定时器
			rtc_status |= RTC_TIMER_ON;
			spin_unlock_irq (&rtc_lock);
		}
		set_rtc_irq_bit(RTC_PIE);	//使能RTC中断
		return 0;
	}
	...
	case RTC_IRQP_SET:	/* Set periodic IRQ rate.	设置RTC中断频率*/
	{
		int tmp = 0;
		unsigned char val;
...
static void mask_rtc_irq_bit(unsigned char bit)
{
	...
	val = CMOS_READ(RTC_CONTROL);  //RTC的控制寄存器
	val &=  ~bit;
