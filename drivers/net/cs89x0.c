...
/* And 2.3.47 had this: */
		...
		for (i = 2; i < CS8920_NO_INTS; i++) {
			if ((1 << i) & lp->irq_map) {
				if (request_irq(i, net_interrupt, 0, dev->name, dev) == 0) {  //注册网卡中断
					dev->irq = i;
...
/* And 2.3.47 had this: */
		...
		ret = request_irq(dev->irq, &net_interrupt, 0, dev->name, dev); //注册网卡中断
		if (ret) {
			if (net_debug)
	...
	if (lp->use_dma) {
		if (lp->isa_config & ANY_ISA_DMA) {
			...
			if (request_dma(dev->dma, dev->name)) {		// 设置DMA
	...
	writereg(dev, PP_BusCTL, ENABLE_IRQ
        ...
        netif_start_queue(dev);  //启动发送队列，公共部份
	if (net_debug > 1)
...
static irqreturn_t net_interrupt(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	while ((status = readword(dev, ISQ_PORT))) {
		...
		case ISQ_TRANSMITTER_EVENT:
			lp->stats.tx_packets++;
			netif_wake_queue(dev);	/* Inform upper layers. 发送包完成 通知*/
			if ((status & (	TX_OK |
					TX_LOST_CRS |
					TX_SQE_ERROR |
					TX_LATE_COL |
...
#endif
			break;
		case ISQ_RX_MISS_EVENT:	// 接收出错，丢包
			lp->stats.rx_missed_errors += (status >>6);
			break;
		case ISQ_TX_COL_EVENT:  // 发送 冲突
...
net_rx(struct net_device *dev)
{
	...
	/* Malloc up new buffer. */
	skb = dev_alloc_skb(length + 2);  //分配一个 sk_buff
	if (skb == NULL) {
	...
	insw(ioaddr + RX_FRAME_PORT, skb_put(skb, length), length >> 1);  // 把收到的包从芯片读入 sk_buff
	if (length & 1)
