...
#define PP_RxCFG 0x0102		/*  Rx Bus config 控制CS8900接收到特定数据包后会引发接收中断。如：设置0103H, 表示收到一上正确的数据报后，会产生一下接收中断。*/
#define PP_RxCTL 0x0104		/*  Receive Control Register 控制CS8900接收特定的数据报。如：设置0d05H，表示接收网络上的广播或者目标地址为本MAC的正确数据包。*/
#define PP_TxCFG 0x0106		/*  Transmit Config Register */
#define PP_TxCMD 0x0108		/*  Transmit Command Register */
#define PP_BufCFG 0x010A	/*  Bus configuration Register */
#define PP_LineCTL 0x0112	/*  Line Config Register 决定CS8900的基本配置和物理接口。如：设置00d3H时，表示为10BASE-T，并使能设备的发送与接收控制位。*/
#define PP_SelfCTL 0x0114	/*  Self Command Register */
#define PP_BusCTL 0x0116	/*  ISA bus control Register 可控制芯片的IO接口的一些操作。如：设置为8017H，表示打开CS8900的中断总控制位。*/
#define PP_TestCTL 0x0118	/*  Test Register */
#define PP_AutoNegCTL 0x011C	/*  Auto Negotiation Ctrl */

#define PP_ISQ 0x0120		/*  Interrupt Status 是网卡芯片的中断状态寄存器，内部映射接收中断状态寄存器和发送中断状态寄存器的内容。*/
...
#define PP_TxCommand 0x0144	/*  Tx Command 发送控制寄存器，如：00C0H，表示网卡芯片在全部数据写入后才开始发送数据。*/
#define PP_TxLength 0x0146	/*  Tx Length 发送数据长度寄存器，发送数据时，先写入发送数据长度，然后把数据通过PORT0写入芯片。*/
...
#define PRODUCT_ID_ADD 0x0002   /*  Address of product ID 芯片ID寄存器*/
