...
int scsi_dispatch_cmd(struct scsi_cmnd *cmd)
{
	 ...
	 */
	timeout = host->last_reset + MIN_RESET_DELAY; // 2*HZ =2s
	...
	scsi_add_timer(cmd, cmd->timeout_per_command, scsi_times_out);  //SCSI命令超时设置
...
void __scsi_done(struct scsi_cmnd *cmd)
{
	...
	raise_softirq_irqoff(SCSI_SOFTIRQ);/* 触发软中断 */  scsi_softirq
 ...
 * drivers.
 */ // scsi_done或 scsi_times_out 处理SCSI命令时，把命令放入scsi_done_q链表，触发软中断
