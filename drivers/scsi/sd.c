...
static int sd_probe(struct device *dev)
{
	...
	/* 设备数量太多，退出 */
	if (index >= SD_MAX_DISKS)  // sda-sdz, sdaa-sdzz, sdaaa,sdzzz,  26+26*26+26*26*26=
		error = -EBUSY;
	if (error)
		goto out_put;
	...
	if (!sdp->timeout) {
		if (sdp->type == TYPE_DISK)
			sdp->timeout = SD_TIMEOUT; //30*HZ  当HZ为1000时，为30MS
		else/* 磁光盘超时时间应当长一点 */
			sdp->timeout = SD_MOD_TIMEOUT;	//75*HZ, 当HZ为1000时，为75MS
	}
	...
	gd->queue = sdkp->device->request_queue;  // SCSI设备的请求队列  由scsi_alloc_queue。 SCSI设备扫描上线时分配
