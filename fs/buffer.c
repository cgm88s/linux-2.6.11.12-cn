...
try_again:
	head = NULL;
	offset = PAGE_SIZE;
	while ((offset -= size) >= 0) { // 为一个page中的每个块都申请一个bh
...
link_dev_buffers(struct page *page, struct buffer_head *head)
{
	...
	attach_page_buffers(page, head);  // 关联pagecache 和 bh
}
 ...
 * bh:		要传送数据的块缓冲区首部。
 */
int submit_bh(int rw, struct buffer_head * bh) //把bh对应的缓存有BIO提交
{
	struct bio *bio;
	int ret = 0;
...
static void recalc_bh_state(void)
{
	...
	buffer_heads_over_limit = (tot > max_buffer_heads);  // 设置 超过最大的bh数
}
