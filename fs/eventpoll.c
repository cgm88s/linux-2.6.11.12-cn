...
struct eventpoll {
	...
	struct list_head rdllist;	 // 已有事件的 文件对象 链表

	/* RB-Tree root used to store monitored fd structs */
	struct rb_root rbr;  // 监听 事件的所有 文件对象的 红黑树
};
...
struct epitem {
	...
	struct list_head rdllink;	// 有事情时链入eventpoll->rdlist

	/* The file descriptor information this item refers to */
	struct epoll_filefd ffd; //对应监听的文件
	...
	struct list_head fllink;  // 添加监听的文件file链表
 ...
 */
asmlinkage long sys_epoll_create(int size)   //创建epoll fd
{
	int error, fd;
	struct inode *inode;
	struct file *file;
	...
	error = ep_file_init(file);  //初始化文件相关的 私有数据 eventpoll
	if (error)
		goto eexit_2;
...
static int ep_getfd(int *efd, struct inode **einode, struct file **efile)
{
	...
	error = -ENFILE;
	file = get_empty_filp();  // 分配一个file 结构体
	if (!file)
		goto eexit_1;
		     ...
		     struct file *tfile, int fd)
{
	 ...
	 */
	revents = tfile->f_op->poll(tfile, &epq.pt);  socket_file_ops->poll=  sock_poll
