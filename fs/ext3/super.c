 ...
 * appropriate. 
 */
 //开始一个日志操作. 获取或分配 一个原子操作handle(与进程关联)
			  ...
			  unsigned long * inum, unsigned long *n_blocks_count, int is_remount)
{
	...
	while ((p = strsep (&options, ",")) != NULL) {
		...
		case Opt_commit:
			...
			break;
		case Opt_data_journal:    // journal日志模式
			data_opt = EXT3_MOUNT_JOURNAL_DATA;
			goto datacheck;
		case Opt_data_ordered:  // ordered日志模式
			data_opt = EXT3_MOUNT_ORDERED_DATA;
			goto datacheck;
		case Opt_data_writeback:  // writeback日志模式
...
static int ext3_fill_super (struct super_block *sb, void *data, int silent)
{
	...
	struct ext3_super_block *es = NULL;	//ext3磁盘上的超级块
	struct ext3_sb_info *sbi;  //ext3内存中的超级块
	unsigned long block;
	...
	if (!sbi)
		return -ENOMEM;
	sb->s_fs_info = sbi;   //分配一个ext3内存超级块，并加入到VFS通用超级块sb对象中
	...
	if (!(bh = sb_bread(sb, logic_sb_block))) {  //读磁盘超级块
	...
	if (!test_opt(sb, NOLOAD) &&
		...
		if (ext3_load_journal(sb, es))   //加载日志kjournald
			goto failed_mount2;
...
static journal_t *ext3_get_journal(struct super_block *sb, int journal_inum)
{
	...
	journal_inode = iget(sb, journal_inum);  //获取日志文件的inode， 8
	if (!journal_inode) {
