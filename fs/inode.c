 ...
 */
struct inodes_stat_t inodes_stat;  //全局系统中的inode统计计数
 ...
 * unlock_new_inode().
 */   
struct inode *iget_locked(struct super_block *sb, unsigned long ino) //从文件系统中获取一下inode
{
	struct hlist_head *head = inode_hashtable + hash(sb, ino);
	struct inode *inode;

	inode = ifind_fast(sb, head, ino); // 在inode_hashtable中查找已经存在的inode缓存
	if (inode)
		return inode;
	/*
	 ...
	 * in case it had to block at any point.
	 */
	return get_new_inode_fast(sb, head, ino); //没有找到就重新分配
}
...
{
	if (inode) {
		struct super_operations *op = inode->i_sb->s_op;   sockfs_ops
