...
int kjournald(void *arg)
{
	...
	journal->j_commit_timer = &timer;   //在 函数 get_transaction 中添加定时器
...
//启动日志线程，线程函数为kjournald, 函数内会把线程名变为kjournald
...
static journal_t * journal_init_common (void)
{
	...
	journal->j_commit_interval = (HZ * JBD_DEFAULT_MAX_COMMIT_AGE);  //默认5s, kjournald的唤醒间隔时间
 ...
 */
journal_t * journal_init_inode (struct inode *inode) //创建一个journal_t 和日志inode关联起来
{
	struct buffer_head *bh;
	journal_t *journal = journal_init_common();  //分配并初始化一个journal_t 日志超级块
	int err;
	unsigned long blocknr;
	...
	err = journal_bmap(journal, 0, &blocknr);  //找到日志inode中0号块所在的磁盘扇区
...
static int journal_reset(journal_t *journal)
{
	...
	journal_start_thread(journal);	//启动日志线程 kjournald
	return 0;
}
 ...
 * structures.
 */
int journal_load(journal_t *journal) //从磁盘读日志，并启动日志线程 kjournald
{
	int err;
