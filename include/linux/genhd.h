...
struct disk_stats {
	unsigned read_sectors, write_sectors; // 读扇区，写扇区
	unsigned reads, writes;	// 读IO个数，写IO个数
	unsigned read_merges, write_merges;	// 读IO个数合并个数(有多少IO可以合并)，写IO个数合并个数
	unsigned read_ticks, write_ticks;	// 读花费时间，写花费时间
	unsigned io_ticks;		//
...
struct gendisk {
	...
	int in_flight;		//请求队列中，请求个数
#ifdef	CONFIG_SMP
	struct disk_stats *dkstats;  //磁盘的统计 计数器
#else
