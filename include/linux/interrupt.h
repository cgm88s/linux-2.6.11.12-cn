...
struct irqaction {  //设备相关的中断服务例程 ISR
	irqreturn_t (*handler)(int, void *, struct pt_regs *); //中断服务例程函数
	unsigned long flags;		//IRQ与设备的关系，IRQ_SHIRQ,IRQ_INTERRUPT
	cpumask_t mask;
	const char *name;    //IO设备名，/proc/interrupts中会显示设备名
	void *dev_id;		//IO设备的私有字段，标识IO设备的本身
	struct irqaction *next;  //指向中断服务例程链表的下一个
	int irq;		//IRQ线
	struct proc_dir_entry *dir; //与IRQn相关的/proc/irq/n目录的描述符
};
