...
#error	"Please fix <asm/byteorder.h>"
	 ...
	 * 报文长度，包括报头和分片。
	 */
	__u16	tot_len;  // 16bit总长度，包括协议头与数据
	/**
	 ...
	 * 分片在原始报文中的偏移。
	 */
	__u16	frag_off;	// 3bit 标识，13bit报文偏移
	/**
	 * TTL，路由器在每次转发时递减此值。
	 */
	__u8	ttl;	// 8bit 生成时间，time to live
	/**
	 * L4层协议标识。
	 */
	__u8	protocol;	// 8bit 协议
	/**
	 * 校验和。
	 */
	__u16	check;	//16bit 头部校验和
	/**
	 * 源地址。
	 */
	__u32	saddr;	// 32bit 源地址
	/**
	 * 目的地址。
	 */
	__u32	daddr; // 32bit 目标地址
