...
//伙伴算法
struct free_area {
	struct list_head	free_list; //新版还会分类
 ...
 * 但是每个管理结点又包含了不同的管理区。
 */
typedef struct pglist_data { //UMA 为单一节点，NUMA为多节点
	/**
	 * 节点管理区描述符数组
	 */
	 ...
	 */
	struct zonelist node_zonelists[GFP_ZONETYPES]; // 3
	/**
	 * 节点中管理区的个数
	 */
	int nr_zones;
...
#if 0 //BITS_PER_LONG == 32 || defined(ARCH_HAS_ATOMIC_UNSIGNED)
/*
