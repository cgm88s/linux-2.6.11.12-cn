...
#define NETIF_F_LLTX		4096	/* LockLess TX */
	...
	int			(*open)(struct net_device *dev);  // ifconfig激活接口时会调用.  注册中断，DMA，设置寄存器，启动设备，启动发送队列
...
#define HAVE_NETDEV_POLL
	...
	/**
	 * 这些函数被邻居层使用。按字段填充L2帧头。用于L2帧头缓冲。根据先前检索到的源和目的硬件地址建立硬件头
	 */
	 ...
	 */	
	int			(*rebuild_header)(struct sk_buff *skb); //完成ARP解析后，重新建立硬件头
