 ...
 */
struct sched_group {
	struct sched_group *next;	/* 下一个 CPU group .Must be a circular list */
	cpumask_t cpumask;	//当前group有那些CPU

	/*
	 * CPU power of this group, SCHED_LOAD_SCALE being max power for a
	 * single CPU. This is read only (except for setup, hotplug CPU).
	 */
	unsigned long cpu_power; //当前group的cpu power
};
...
struct sched_domain {
	...
	/**
	 * 组描述符的第一个元素。CPU group。
	 */
	struct sched_group *groups;	/* the balancing groups of the domain */
	cpumask_t span;			/* 当前域中所有CPU的位图 span of all CPUs in this domain */
	unsigned long min_interval;	/*最小load balance间隔 Minimum balance interval ms */
	unsigned long max_interval;	/*最大的load balance 间隔 Maximum balance interval ms */
	unsigned int busy_factor;	/* busy时延迟进行balance的系数 less balancing by factor if busy */
	unsigned int imbalance_pct;	/* No balance until over watermark */
	unsigned long long cache_hot_time; /* Task considered cache hot (ns) */
	unsigned int cache_nice_tries;	/* Leave cache hot tasks for # tries */
	unsigned int per_cpu_gain;	/* CPU % gained by adding domain cpus */
	int flags;			/* See SD_* 当前域的状态标记 */

	/* Runtime fields. */
	unsigned long last_balance;	/*当前域最近进行balance时的时间  init to jiffies. units in jiffies */
	unsigned int balance_interval;	/*进行balance的时间间隔 initialise to 1. units in ms. */
	unsigned int nr_balance_failed; /* balance失败的次数 initialise to 0 */
...
struct task_struct {
	 ...
	 * mm:指向内存区描述符的指针
	 */
	struct mm_struct *mm, *active_mm;//用户进程这两个相等，内核进程，mm=NULL,active_mm则为上一个用户进程序的mm
...
/**
 * 内核栈与thread_info的联合体。
 *如果内核编译时，thread_union为8K，内核栈就用于: 异常，中断，软中断
 *否则内核使用三种类型的内核栈:导常栈，硬中断栈 hardirq_ctx，软中断栈 softirq_ctx
 */
union thread_union { 
	struct thread_info thread_info;
...
static inline int need_resched(void)	// 测试current->thread->flags 的 TIF_NEED_RESCHED,设置表示需要调度
{
