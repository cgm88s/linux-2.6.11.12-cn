...
/* TCP首部 */
struct tcphdr {		// tcp协议头  20个字节byte
	__u16	source;  	// 源端口     2 byte
	__u16	dest;		// 目标端口	2 byte
	__u32	seq;		// seq序列号	4 byte
	__u32	ack_seq;	// ack序列号	4 byte
...
#endif	
	__u16	window;		//窗口	2byte
	__u16	check;		//校验和	2byte
	__u16	urg_ptr;	//紧急指针 2byte
};
