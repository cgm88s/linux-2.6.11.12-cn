...
static void __init smp_init(void)
{ // 1号内核进程 调用，由启动CPU 0 用来唤醒其它的CPU
	unsigned int i;

	/* FIXME: This should be done in userspace --RR */
	for_each_present_cpu(i) {  // 循环 遍历 所有的CPU
		if (num_online_cpus() >= max_cpus) // SMP时为8，没有开SMP时为1
			break;
		if (!cpu_online(i))
			cpu_up(i); //唤醒 指定的CPU
	}
 ...
 */
// 由start_kernel调用，作为0号进程，
static void noinline rest_init(void)
	__releases(kernel_lock)
{
	kernel_thread(init, NULL, CLONE_FS | CLONE_SIGHAND);	//创建init 1号进程
	numa_default_policy();
	unlock_kernel();
 ...
 */
	...
	page_address_init();
	printk(linux_banner);
	setup_arch(&command_line); //架构相关的初始化，启动时初始化
	setup_per_cpu_areas();
	...
	parse_args("Booting kernel", command_line, __start___param,
		   ...
		   &unknown_bootoption);
	sort_main_extable();
	trap_init();	//初始化异常		中断描述表
	rcu_init();
	init_IRQ();		//初始化中断		中断描述表
	pidhash_init();
	init_timers();
	softirq_init();
	time_init();
...
static int __initdata initcall_debug; //内核启动参数，用于跟踪系统启动过程中顺序
...
//init进程初始化，跟据vmlinux.lds里initcall的级别顺序调用，
...
static int init(void * unused)
{
	 ...
	 */
	//最终调用用户态的init进程，从此，1号进程进入用户态
	if (execute_command)
		run_init_process(execute_command); //  /init
