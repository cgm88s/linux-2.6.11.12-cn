 ...
 */
/**
 * CPUÈÎÎñÔËĞĞ¶ÓÁĞ:Ã¿¸öCPUºË¶¼ÓĞÒ»¸ö£¬per-cpu±äÁ¿
 */
struct runqueue {
	/**
	 * ±£»¤½ø³ÌÁ´±íµÄ×ÔĞıËø
 ...
 */ //°ÑÒ»¸öĞÂµÄ½ø³Ì»òÏß³Ì ¼ÓÈëµ½µ±Ç°µÄCPUµÄ »î¶¯½ø³ÌÁ´±í£¬ rq->nr_running++;
...
{
	rq->nr_running--;
	dequeue_task(p, p->array);		// °Ñ ½ø³Ì p ´ÓÔËĞĞ¶ÓÁĞ p->arrayÖĞÒÆ³ı
	p->array = NULL;
}
...
unsigned long nr_running(void)
{
	...
	for_each_online_cpu(i)
		sum += cpu_rq(i)->nr_running; //runqueues->nr_running  Ã¿¸öCPUÖĞ¿ÉÔËĞĞ½ø³ÌµÄÊıÁ¿ R 
...
unsigned long nr_uninterruptible(void)
{
	...
	for_each_cpu(i)
		sum += cpu_rq(i)->nr_uninterruptible; //runqueue->nr_uninterruptible  Ã¿¸öCPUÖĞ²»¿ÉÖĞ¶ÏË¯Ãß½ø³ÌµÄÊıÁ¿ D
...
/* ÕÒ³öµ±Ç°µ÷¶ÈÓòsdÖĞ×îÃ¦µÄµ÷¶È×ésg
...
nextgroup:
	...
	avg_load = (SCHED_LOAD_SCALE * total_load) / total_pwr; //Æ½¾ù¸ºÔØ
				...
				struct sched_domain *sd)
{
	...
	group = find_busiest_group(sd, this_cpu, &imbalance, NEWLY_IDLE); //´Óµ±Ç°µ÷¶ÈÓòsdÖĞÕÒµ½×îÃ¦µÄµ÷¶È×ésg
	if (!group) {
		schedstat_inc(sd, lb_nobusyg[NEWLY_IDLE]);
		goto out;
	}

	busiest = find_busiest_queue(group); //´Óµ÷¶È×ésgÖĞÕÒµ½×îÃ¦µÄµ÷¶È¶ÓÁĞrq
...
/* µ±±¾CPUÉÏÔËĞĞ¶ÓÁĞÃ»ÓĞ¿ÉÔËĞĞµÄtask,¾Í´ÓÆäËüµÄCPUÉÏÇ¨ÒÆÒ»Ğ©¿ÉÔËĞĞµÄtaskµ½±¾µØrq
...
static inline void idle_balance(int this_cpu, runqueue_t *this_rq)
{
	...
	for_each_domain(this_cpu, sd) {  //´ÓÏÂµ½ÉÏ±éÀúËùÓĞCPU µ÷¶ÈÓò
...
need_resched_nonpreemptible:
	rq = this_rq();     // È¡µ±Ç°CPUÉÏµÄÔËĞĞ¶ÓÁĞ runqueues
	...
	schedstat_inc(rq, sched_cnt);   // µ÷¶È¼ÆÊı¼Ó 1, rq->sched_cnt++
	/**
	 * ¼ÆËãµ±Ç°½ø³ÌµÄÔËĞĞÊ±¼ä¡£²»³¬¹ı1Ãë¡£
	 */
	now = sched_clock(); //¼ÆËãµ±Ç°µÄÊ±¼ä
...
go_idle:
	 ...
	 */
	idx = sched_find_first_bit(array->bitmap);    // ´ÓÎ»Í¼ÖĞÕÒµ½µÚ1¸öÖÃÎ»µÄ
	queue = array->queue + idx;					 // ×îµ½×î¸ßÓÅÏÈ¼¶µÄqueue
	/**
	 * ½«ÏÂÒ»¸ö¿ÉÔËĞĞ½ø³ÌÃèÊö·û·Åµ½nextÖĞ
	 */
	next = list_entry(queue->next, task_t, run_list);			// ´Ó×î¸ßÓÅÏÈ¼¶µÄqueueÖĞÈ¡µÚÒ»¸ö½ø³Ì
...
//µ÷¶ÈÆ÷³õÊ¼»¯: O(0)µ÷¶È,ĞÂ°æÉÏÊ¹ÓÃCFGµ÷¶ÈÆ÷,struct runqueue rq;
