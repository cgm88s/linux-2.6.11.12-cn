 ...
 */
struct timespec xtime __attribute__ ((aligned (16)));	//墙上时间
...
void update_process_times(int user_tick)
{
	 ...
	 * 而用户的定时器是通过信号来实现的。
	 */
	run_local_timers(); // --> run_timer_softirq
	/**
 ...
 * 但是这也说明在某些情况下，有反例出现。
 * 被update_times在每个节拍调用。指数平滑法 http://kurtlau.github.io/2013/04/14/load-avg/
 */
static inline void calc_load(unsigned long ticks)
{
	unsigned long active_tasks; /* fixed-point */
	static int count = LOAD_FREQ;	// 5*HZ	 5秒中tick次数

	count -= ticks;
	if (count < 0) {
		count += LOAD_FREQ;
		active_tasks = count_active_tasks();	// (R+D)*(1<<11)
...
{		// 2.6.16以上引入hpet后，此处会有 hrtimer_run_queues
...
void __init init_timers(void)
{
	...
	open_softirq(TIMER_SOFTIRQ, run_timer_softirq, NULL); //由 update_process_timer 唤醒
}
