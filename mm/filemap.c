...
find_page:
		...
		/**
		 * 运行到此,说明页已经位于页高速缓存中,检查标志PG+uptodate.
...
page_not_up_to_date:
		...
		/**
		 * lock_page获取对页的互斥访问.如果PG_locked已经置位,则lock_page会阻塞进程,直到标志被清0.
		 */
		lock_page(page);
...
readpage:
		 ...
		 */
		error = mapping->a_ops->readpage(filp, page);  ext2_aops; ext3_ordered_aops
...
out:
	/**
	 * 所有请求的或者说可以读到的数据已经读完,就更新filp->f_ra,标记数据已经被顺序从文件读入(参见文件预读).
	 */	
	*_ra = ra;
				...
				unsigned long nr_segs, loff_t *ppos)
{
	...
	/* coalesce the iovecs and go direct-to-BIO for O_DIRECT */
	if (unlikely(file->f_flags & O_DIRECT)) {	// O_DIRECT直接写
