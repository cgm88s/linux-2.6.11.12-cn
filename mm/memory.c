...
unsigned long num_physpages;  //物理内存的总页数，3.10.0以上叫做 totalram_pages
/*
...
static inline void clear_pmd_range(struct mmu_gather *tlb, pmd_t *pmd, unsigned long start, unsigned long end)
{
	...
	if (!((start | end) & ~PMD_MASK)) {
		...
		page = pmd_page(*pmd);
		pmd_clear(pmd);
		dec_page_state(nr_page_table_pages); //减少当前CPU上 struct page_state的pagetable的计数
		tlb->mm->nr_ptes--;
...
//如果此pmd表项存在，就作为pte的基地址，返回address对应的pte表项，如果pmd表项不存在，就重新分配一个页，作为pte
pte_t fastcall * pte_alloc_map(struct mm_struct *mm, pmd_t *pmd, unsigned long address)
{
	if (!pmd_present(*pmd)) {
		...
		new = pte_alloc_one(mm, address);  //get_zeroed_page(GFP_KERNEL|__GFP_REPEAT); 分配一个页
		...
		mm->nr_ptes++;
		inc_page_state(nr_page_table_pages); //增加当前CPU中 struct page_state的pagetable的计数, 页表项计数只包含pte的页
		pmd_populate(mm, pmd, new);  // pmd.pmd= new页框号+标志
	}
out:
	return pte_offset_map(pmd, address); pte_offset_kernel   //返回PMD中对应ADDRESS的项 =pte
}
...
retry:
	...
	update_mmu_cache(vma, address, entry);   //NOP
	spin_unlock(&mm->page_table_lock);
out:
	return ret;
oom:
		...
		unsigned long address, int write_access)
{
	...
	inc_page_state(pgfault);  // page_states.pgfault 计数+1  stuct page_state
 
	if (is_vm_hugetlb_page(vma))	// VM_HUGETLB 表明使用大页
	 ...
	 */
	pgd = pgd_offset(mm, address);  //mm->pgd+index  从address获取pgd表项索引，获取address对应的PGD
	spin_lock(&mm->page_table_lock);

	pud = pud_alloc(mm, pgd, address); // 如果此pgd表项存在，就作为pud的基地址，返回address对应的pud表项。如果pgd表项不存在，就重新分顶一个页，作为pud
	if (!pud)
		goto oom;

	pmd = pmd_alloc(mm, pud, address);// 如果此pud表项存在，就作为pmd的基地址，返回address对应的pmd表项。如果pud表项不存在，就重新分顶一个页，作为pmd
	if (!pmd)
		goto oom;

	pte = pte_alloc_map(mm, pmd, address); //如果此pmd表项存在，就作为pte的基地址，返回address对应的pte表项，如果pmd表项不存在，就重新分配一个页，作为pte
	if (!pte)
		goto oom;
...
pud_t fastcall *__pud_alloc(struct mm_struct *mm, pgd_t *pgd, unsigned long address)
{
	...
	spin_unlock(&mm->page_table_lock);
	new = pud_alloc_one(mm, address);  //获取一个空page，用于pud
	spin_lock(&mm->page_table_lock);
	if (!new)
		return NULL;
	 ...
	 * entry, as somebody else could have populated it..
	 */
	if (pgd_present(*pgd)) { //有可能在申请page时，其它进程已重新映射了此表项
		pud_free(new);
		goto out;
	}
	pgd_populate(mm, pgd, new); //pgd页表项的值 指向一个新的页，新页用来装pud表项
 out:
