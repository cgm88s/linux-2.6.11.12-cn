...
static int __pdflush(struct pdflush_work *my_work)
{
	...
	for ( ; ; ) {
		...
		schedule();	//由pdflush_operation 添加工作，并唤醒
 ...
 */
static int pdflush(void *dummy) //回写线程pdflush
{
...
//回写线程:pdflush,新版上为default_bdi_init,bdi_default线程
static int __init pdflush_init(void)
{
	int i;

	for (i = 0; i < MIN_PDFLUSH_THREADS; i++)	//pdflush线程最少2个，最多8个
		start_one_pdflush_thread();
	return 0;
}
