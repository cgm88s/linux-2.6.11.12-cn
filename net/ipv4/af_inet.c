...
static int inet_create(struct socket *sock, int protocol)
{
	...
	/* 设置套接口层的接口 */
	sock->ops = answer->ops;
	answer_prot = answer->prot;	//三种协议类型: tcp_prot,udp_prot, raw_prot
	...
	sk = sk_alloc(PF_INET, GFP_KERNEL,
		      ...
		      answer_prot->slab);   // tcp_sock, udp_sock, raw_sock
	if (sk == NULL)
		goto out;
...
int inet_shutdown(struct socket *sock, int how)
{
	...
	default:
		sk->sk_shutdown |= how;
		if (sk->sk_prot->shutdown)/* 正常状态下调用传输层的shutdown */   udp_prot, tcp_prot
...
struct proto_ops inet_stream_ops = {	//传输层接口方法
	.family =	PF_INET,
	.owner =	THIS_MODULE,
...
void inet_register_protosw(struct inet_protosw *p)  //注册协议类型 --> 全局链表数据 inet_sw
{
	struct list_head *lh;
	struct inet_protosw *answer;
