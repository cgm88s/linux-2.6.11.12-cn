...
static inline int ip_local_deliver_finish(struct sk_buff *skb)
{
	...
	resubmit:
		 ...
		 */
		if ((ipprot = rcu_dereference(inet_protos[hash])) != NULL) {  //l4´«Êä²ãÐ­Òé icmp_protocol udp_protocol  tcp_protocol igmp_protocol
			int ret;
			 ...
			 * µ÷ÓÃL4²ã´¦Àíº¯Êý¡£
			 */
			ret = ipprot->handler(skb);  //L4²ã´«Êä²ãÐ­ÒéÈë¿Ú  icmp_rcv, udp_rcv,tcp_v4_rcv, igmp_rcv
...
int ip_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt)
{
	...
	return NF_HOOK(PF_INET, NF_IP_PRE_ROUTING, skb, dev, NULL,
		       ip_rcv_finish);			//pre_routing ·À»ðÇ½¹ýÂËµã
