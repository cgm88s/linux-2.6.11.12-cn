			  ...
			  struct tcphdr *th, unsigned len)
{
	...
	case TCP_LISTEN:/* TCP_LISTEN×´Ì¬£¬Ö»´¦ÀíSYN¶Î */
		...
		if(th->syn) {/* ´¦ÀíSYN±¨ÎÄÇëÇó */
			...
			if(tp->af_specific->conn_request(sk, skb) < 0)			ipv4_specific.conn_request= tcp_v4_conn_request   //·¢ËÍ ACK+SYN,µÚ2¸öÎÕÊÖ°ü
				return 1;
	...
	if (th->ack) {/* Èç¹ûÓĞACK±êÖ¾ */
		...
		case TCP_FIN_WAIT1:/* ´¦ÀíFIN_WAIT1×´Ì¬ÏÂ½ÓÊÕµ½µÄACK */
			if (tp->snd_una == tp->write_seq) {/* Í¨¹ıACK¶ÎµÄÈ·ÈÏ£¬ËùÓĞ·¢ËÍ¶Î¶Ô·½¶¼ÒÑ¾­ÊÕµ½£¬ÔòÇ¨ÒÆµ½FIN_WAIT2×´Ì¬ */
				tcp_set_state(sk, TCP_FIN_WAIT2);  //°ÑsocketµÄ×´Ì¬ÖÃÎª TCP_FIN_WAIT2
				sk->sk_shutdown |= SEND_SHUTDOWN;
				dst_confirm(sk->sk_dst_cache);/* ´Ó¶Ô·½ÊÕµ½ACK¶Î£¬Òò´Ë¿ÉÒÔÈ·ÈÏ´ËÂ·ÓÉ»º´æÓĞĞ§ */

				if (!sock_flag(sk, SOCK_DEAD))/* ²»ÔÚDEAD×´Ì¬²¢ÇÒ×´Ì¬·¢ÉúÁË±ä»¯£¬Í¨ÖªµÈ´ıµÄÏß³Ì */
					...
					/* ÔÚFIN_WAIT2µÈ´ı */
					tmo = tcp_fin_time(tp);
					if (tmo > TCP_TIMEWAIT_LEN) {    // ´óÓÚ 60s
						tcp_reset_keepalive_timer(sk, tmo - TCP_TIMEWAIT_LEN);
					} else if (th->fin || sock_owned_by_user(sk)) {   //FINÓëACKÍ¬Ê±ÉèÖÃ»òµ±Ç°ÊÇ½ø³ÌÉÏÏÂÎÄ
