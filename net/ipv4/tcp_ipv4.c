...
static inline void syn_flood_warning(struct sk_buff *skb)
{
	...
	//当前时间，jiffies > warntime(上次时间)+60s
	if (time_after(jiffies, (warntime + HZ * 60))) {  // 在60s之内，再次进入，才打印
		warntime = jiffies;
		printk(KERN_INFO
...
static int tcp_v4_init_sock(struct sock *sk)
{
	...
	sk->sk_rcvbuf = sysctl_tcp_rmem[1];  //用tcp_rmem的default值再重新初始化
