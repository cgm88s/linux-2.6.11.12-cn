...
static DECLARE_WAIT_QUEUE_HEAD(nl_table_wait); struct wait_queue_head_t*nl_table_wait;

static int netlink_dump(struct sock *sk);
static void netlink_destroy_callback(struct netlink_callback *cb);

static DEFINE_RWLOCK(nl_table_lock); rwlock_t nl_table_lock = RW_LOCK_UNLOCKED;
 ...
 *	queueing.
 	netlink内核服务器端实现。每个协议都要实现:
 		NETLINK_ROUTTE,	--> rtnetlink_rcv
 		NETLINK_NETFILTER,  -->
 	用户态创建netlink时，指定protocol,就是与具体的服务器通信
 */
