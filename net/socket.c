 ...
 *	NULL is returned.
 */
//分配一个socket,与之相关inode
static struct socket *sock_alloc(void)
{
	struct inode * inode;
	struct socket * sock;

	inode = new_inode(sock_mnt->mnt_sb);  sockfs_ops  sock_alloc_inode
	if (!inode)
		return NULL;
...
void sock_release(struct socket *sock)
{
	if (sock->ops) {/* 存在传输层接口 */
		...
		sock->ops->release(sock);/* 调用release释放传输控制块，如inet_release */  socket_file_ops,sock_close
		sock->ops = NULL;
...
static long sock_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		default:
			 ...
			 */
			err = sock->ops->ioctl(sock, cmd, arg);  inet_dgram_ops->ioctl=inet_ioctl
			break;
	}
	return err;
}
...
static unsigned int sock_poll(struct file *file, poll_table * wait)
{
	 ...
	 */
	sock = SOCKET_I(file->f_dentry->d_inode);
	return sock->ops->poll(file, sock, wait);  inet_stream_ops  tcp_poll
}
...
static int __sock_create(int family, int type, int protocol, struct socket **res, int kern)
{
	 ...
	 */
	if (family < 0 || family >= NPROTO)/* 参数合法性检测 */  //常用的协议家族有:AF_UNIX,AF_LOCAL, AF_INET,AF_NETLINK
		return -EAFNOSUPPORT;
	if (type < 0 || type >= SOCK_MAX)      //常用有类型有:SOCK_STREAM， SOCK_DGRAM , SOCK_RAM,SOCK_PACKET
		return -EINVAL;
 ...
 */
	...
	/* 调用协议族的创建方法，对IPV4来说，调用的是 inet_create, AF_NETLINK=netlink_create */
	if ((err = net_families[family]->create(sock, protocol)) < 0)  // unix_create,inet_create
		goto out_module_put;
	/*
...
asmlinkage long sys_listen(int fd, int backlog)
{
	...
	if ((sock = sockfd_lookup(fd, &err)) != NULL) {/* 根据文件描述符获得套接口指针 */
		if ((unsigned) backlog > sysctl_somaxconn)/* 最大连接数量，超过此数量的未accept连接将被丢弃 */
			backlog = sysctl_somaxconn;				//net.core.somaxconn，最大连接数，
...
asmlinkage long sys_shutdown(int fd, int how)
{
	...
	if ((sock = sockfd_lookup(fd, &err))!=NULL)
	{
		...
		err=sock->ops->shutdown(sock, how);/* 对IPV4来说，是inet_shutdown */  inet_stream_ops, inet_dgram_ops
		sockfd_put(sock);
	}
	return err;
}
